...
import {
  ...
  MojangRestAPI,
  Session,
} from "../../../lib/mojang/rest/MojangRestAPI";
import { expect } from "chai";
import nock from "nock";
import {
  MojangErrorCode,
  MojangResponse,
} from "../../../lib/mojang/rest/MojangResponse";
import {
  assertResponse,
  expectFailure,
  expectSuccess,
} from "../../common/RestResponseUtil";

function expectMojangResponse(
  res: MojangResponse<unknown>,
  responseCode: MojangErrorCode,
  negate = false
): void {
  assertResponse(res);
...
describe("[Mojang Rest API] Status", () => {
  it("Status (Online)", async () => {
    ...
    nock(MojangRestAPI.STATUS_ENDPOINT).get(/.*/).reply(200, summaryResponse);
...
describe("[CUSTOM Rest API] Auth", () => {
  ...
  it("Validate", async () => {
    nock(MojangRestAPI.AUTH_ENDPOINT)
      ...
      .reply((uri, requestBody: any) => {
        return [requestBody.accessToken === "abc" ? 204 : 403];
      });
  ...
  it("Invalidate", async () => {
    nock(MojangRestAPI.AUTH_ENDPOINT).post("/invalidate").reply(204);
