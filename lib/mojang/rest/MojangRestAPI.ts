...
export class MojangRestAPI {
  ...
  public static readonly AUTH_ENDPOINT = "https://authmc.remmody.ru/auth";
   ...
   */
  private static handleGotError<T>(
    operation: string,
    error: RequestError,
    dataProvider: () => T
  ): MojangResponse<T> {
    const response: MojangResponse<T> = handleGotError(
      operation,
      error,
      MojangRestAPI.logger,
      dataProvider
    );

    if (error instanceof HTTPError) {
      response.mojangErrorCode = decipherErrorCode(
        error.response.body as MojangErrorBody
      );
  ...
  private static expectSpecificSuccess(
    ...
    if (actual !== expected) {
      MojangRestAPI.logger.warn(
        `${operation} expected ${expected} response, received ${actual}.`
      );
    }
  }
  ...
  public static async status(): Promise<MojangResponse<MojangStatus[]>> {
    try {
      ...
      for (const status of res.body) {
        for (const mojStatus of MojangRestAPI.statuses) {
          if (mojStatus.service === status.slug) {
            mojStatus.status =
              status.status === "up"
                ? MojangStatusColor.GREEN
                : MojangStatusColor.RED;
            break;
          }
        }
      }
      ...
      return MojangRestAPI.handleGotError(
        "Mojang Status",
  ...
  public static async authenticate(
    ...
    try {
      ...
      MojangRestAPI.expectSpecificSuccess(
        "Mojang Authenticate",
        200,
        res.statusCode
      );
  ...
  public static async validate(
    accessToken: string,
    clientToken: string
  ): Promise<MojangResponse<boolean>> {
    try {
      const json = {
        accessToken,
        clientToken,
  ...
  public static async invalidate(
    accessToken: string,
    clientToken: string
  ): Promise<MojangResponse<undefined>> {
    try {
      const json = {
        accessToken,
        clientToken,
  ...
  public static async refresh(
    ...
    try {
      ...
      const res = await MojangRestAPI.authClient.post<Session>("refresh", {
        json,
        responseType: "json",
      });
      MojangRestAPI.expectSpecificSuccess(
